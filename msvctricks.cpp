...
static DWORD run( LPWSTR lpCmdLine ) {
    ...
    DWORD dwExitCode;
    if ( CreateProcessW( nullptr, lpCmdLine, nullptr, nullptr, TRUE, 0, nullptr,
                         nullptr, &si, &pi ) ) {
        if ( hChildJob ) {
...
int WINAPI wWinMain( HINSTANCE hInstance,
    ...
    wchar_t buf[ 32768 ];
    if ( GetEnvironmentVariableW( L"WINE_MSVC_STDIN", buf,
                                  ARRAYSIZE( buf ) ) ) {
        ...
        hStdIn =
            CreateFileW( buf, GENERIC_READ,
                         FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
                         &attr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr );
    }
    if ( GetEnvironmentVariableW( L"WINE_MSVC_STDOUT", buf,
                                  ARRAYSIZE( buf ) ) ) {
        SECURITY_ATTRIBUTES attr = {};
        attr.nLength = sizeof( attr );
        attr.bInheritHandle = TRUE;
        ...
        hStdOut =
            CreateFileW( buf, GENERIC_WRITE,
                         FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
                         &attr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr );
    }
    if ( GetEnvironmentVariableW( L"WINE_MSVC_STDERR", buf,
                                  ARRAYSIZE( buf ) ) ) {
        SECURITY_ATTRIBUTES attr = {};
        attr.nLength = sizeof( attr );
        attr.bInheritHandle = TRUE;
    ...
    if ( hChildJob = CreateJobObjectW( nullptr, nullptr ) ) {
        ...
        SetInformationJobObject( hChildJob, JobObjectExtendedLimitInformation,
                                 &info, sizeof( info ) );
    }
